...
SetPrefixCommand::SetPrefixCommand() : IMCCommand("setprefix", "Set the prefix for Lunes commands", "<prefix>") {
...
bool SetPrefixCommand::execute(std::vector<std::string>* args) {
	...
	clientMessageF("[%sLunes%s] %sSet prefix to %s%c", GOLD, WHITE, GREEN, GRAY, prefix);
	return true;
}
