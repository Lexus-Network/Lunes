...
HudModule::HudModule() : IModule(0, Category::GUI, "Displays ArrayList/TabGUI") {
...
void HudModule::drawKeystroke(char key, const vec2_t& pos) {
	...
	//	DrawUtils::fillRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(16, 16, 16) : MC_Color(80, 80, 80), 1.f);
...
void HudModule::drawLeftMouseKeystroke(vec2_t pos) {
	...
	vec4_t rectPos(
		...
		pos.x + 31.f,
		pos.y + 22.f);
	//DrawUtils::fillRectangle(rectPos, GameData::GameData::isLeftClickDown() ? MC_Color(16, 16, 16) : MC_Color(80, 80, 80), 1.f);
	vec2_t textPos(
		(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
		rectPos.y + 10.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
	DrawUtils::drawText(textPos, &keyString, MC_Color(255, 255, 255), 1.f, 1.f);
...
void HudModule::drawRightMouseKeystroke(vec2_t pos) {
	...
	vec4_t rectPos(
		...
		pos.x + 31.f,
		pos.y + 22.f);
	//DrawUtils::fillRectangle(rectPos, GameData::GameData::isRightClickDown() ? MC_Color(16, 16, 16) : MC_Color(80, 80, 80), 1.f);
	vec2_t textPos(
		(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
		rectPos.y + 10.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
	DrawUtils::drawText(textPos, &keyString, MC_Color(255, 255, 255), 1.f, 1.f);
...
void HudModule::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	{  // FPS
		if (!(g_Data.getLocalPlayer() == nullptr || !fps)) {
			...
			vec4_t rectPos = vec4_t(2.5f, startY + 10.f * scale, len, startY + 15.f * scale);
	...
	{  // Coordinates
		if (!(g_Data.getLocalPlayer() == nullptr || !coordinates)) {
			...
			vec4_t rectPos = vec4_t(2.5f, startY + 10.f * scale, len, startY + 35.f * scale);
			vec2_t textPos = vec2_t(rectPos.x + 1.5f, rectPos.y + 1.f);
			//DrawUtils::fillRectangle(rectPos, MC_Color(12, 12, 12), 1.f);
