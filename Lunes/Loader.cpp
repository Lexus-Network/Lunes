...
DWORD WINAPI injectorConnectionThread(LPVOID lpParam) {
	...
	auto** LunesToInjectorPtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues));
	auto** injectorToLunesPtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues) + sizeof(uintptr_t));

	*LunesToInjectorPtr = new MemoryBoi();
	MemoryBoi* LunesToInjector = *LunesToInjectorPtr;
	LunesToInjector->isPresent = true;
	LunesToInjector->protocolVersion = 1;

	*injectorToLunesPtr = new MemoryBoi();
	MemoryBoi* injectorToLunes = *injectorToLunesPtr;

	magicArray[0] = 0x48;  //Only find this allocated one, not the one in the thread stack

	logF("LunesToInjectorPtr at %llX", LunesToInjector);
	logF("injectorToLunesPtr at %llX", injectorToLunes);
	...
	while (isRunning) {
		...
		bool isConnected = LunesToInjector->isPresent && injectorToLunes->isPresent && LunesToInjector->protocolVersion >= injectorToLunes->protocolVersion;

		if (isConnected && !injectorToLunes->isUnread) {
			...
			if (realElapsed > 4.f) {
				...
				injectorToLunes->isPresent = false;
				QueryPerformanceCounter(&timeSinceLastMessage);
			}
		}
		...
		if (isConnected) {
			...
			if (injectorToLunes->isUnread) {  // They sent us a message
				QueryPerformanceCounter(&timeSinceLastMessage);
				switch (injectorToLunes->cmd) {
				case CMD_INIT: {
					logF("Got CMD_INIT from injector");
					loggedIn = true;
					int flags = injectorToLunes->params[0];
					if (flags & (1 << 0) && injectorToLunes->dataSize > 0 && injectorToLunes->dataSize < sizeof(injectorToLunes->data)) {  // Has Json data
						injectorToLunes->data[sizeof(injectorToLunes->data) - 1] = '\0';
						json data = json::parse(reinterpret_cast<char*>(injectorToLunes->data));
				...
				case CMD_PING: {
					LunesDataPacket pongPacket;
					pongPacket.cmd = CMD_PONG;
					pongPacket.params[0] = injectorToLunes->params[0];
				...
				case CMD_RESPONSE: {
					int id = injectorToLunes->params[1];

					auto pk = std::make_shared<LunesDataPacket>();
					pk->cmd = (DATAPACKET_CMD)injectorToLunes->params[0];
					memcpy(pk->params, &injectorToLunes->params[2], sizeof(injectorToLunes->params) - sizeof(int) * 2);

					if (injectorToLunes->dataSize > 0 && injectorToLunes->dataSize < 3000) {
						pk->dataArraySize = injectorToLunes->dataSize;

						auto dataTemp = new unsigned char[injectorToLunes->dataSize + 2];
						memset(dataTemp + injectorToLunes->dataSize, 0, 2);  // If we don't zero the last 2 bytes, printing as unicode string won't work
						memcpy(dataTemp, injectorToLunes->data, injectorToLunes->dataSize);
				...
				case CMD_LOG: {
					if (injectorToLunes->dataSize > 4 && injectorToLunes->dataSize < 2999) {
						injectorToLunes->data[injectorToLunes->dataSize] = 0;  // null terminator

						char* command = reinterpret_cast<char*>(&injectorToLunes->data[3]);
				...
				injectorToLunes->isUnread = false;
			}
			...
			if (!LunesToInjector->isUnread && !g_Data.isPacketToInjectorQueueEmpty()) {
				QueryPerformanceCounter(&timeSinceLastMessage);
				// They read the message, lets send the next one
				LunesDataPacket nextDataPack = g_Data.getPacketToInjector();
				if (nextDataPack.dataArraySize >= 3000) {
					throw std::exception("Lunes Data packet too big to send");
				}

				LunesToInjector->cmd = nextDataPack.cmd;
				memcpy(LunesToInjector->params, nextDataPack.params, sizeof(int) * 5);
				if (nextDataPack.dataArraySize > 0) {
					memcpy(LunesToInjector->data, nextDataPack.data.get(), nextDataPack.dataArraySize);
				}

				LunesToInjector->dataSize = nextDataPack.dataArraySize;

				LunesToInjector->isUnread = true;
			}
		} else
			Sleep(30);
	}
	logF("Quitting connection thread");
	LunesToInjector->isPresent = false;
	memset(magicValues, 0, sizeof(magicValues));
	memset(magicArray, 0, sizeof(magicValues) + sizeof(uintptr_t) * 2);
	Sleep(150);
	delete *LunesToInjectorPtr;
	delete *injectorToLunesPtr;
	delete[] magicArray;
