...
void TabGui::renderLevel() {
	...
	for (auto it = labelList.begin(); it != labelList.end(); ++it, i++) {
		...
		vec4_t rectPos = vec4_t(
			xOffset - 4.5f,  // Off screen / Left border not visible 3.5
			yOffset,
			xOffset + maxLength + 10.5f,  //8.5
			yOffset + textHeight);        // y level?
		...
		if (selected[renderedLevel].selectedItemId == i && level >= renderedLevel) {  // We are selected
			if (renderedLevel == level) {                                             // Are we actually in the menu we are drawing right now?
				...
				//	DrawUtils::fillRectangle(rectPos, MC_Color(80, 80, 80), .4); // keep off we still testin
			}
		...
		DrawUtils::drawText(vec2_t(xOffset + 1.5f, yOffset + 10.5f), &tempLabel, label.enabled ? MC_Color() : color, textSize);  // 1.5 10.0 where to render the text part under watermark
	...
	// Draw selected item
	{
		...
		vec4_t selectedPos = vec4_t(
			xOffset - 3.5f,  // Off screen / Left border not visible 0.5
			selectedYOffset,
			xOffset + maxLength + 10.5f,  //8.5
	...
	DrawUtils::flush();
	labelList.clear();
	xOffset += maxLength + 10.5f;  //7.5 wasnt it
	yOffset = selectedYOffset;     // render more
	renderedLevel++;
}
...
void TabGui::render() {
	...
	if (!GameData::canUseMoveKeys())
		level = -1;
	renderedLevel = 0;
	yOffset = 10;  // 10 so it renders under the watermark
	xOffset = 3;   // 3 stay plz
