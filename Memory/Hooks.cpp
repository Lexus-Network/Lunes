...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Lunes prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
		...
		static auto TheMarkMod = moduleMgr->getModule<WaterMark>();
...

		{
			...
			if (strcmp(screenName.c_str(), "start_screen") == 0) {
				// Draw BIG epic Lunes watermark
				{
					std::string text = "L U N E S";
					...
					DrawUtils::fillRectangle(rectPos, MC_Color(80,80,80, 1), 1.f);
				...
				if (g_Data.allowWIPFeatures() && g_Data.isInjectorConnectionActive()) {
					if (HImGui.Button("Load Script Folder", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
						...
						packet.params[0] = g_Data.addInjectorResponseCallback([](std::shared_ptr<LunesDataPacket> pk) {
...

				{
					...
					if (shouldRenderWatermark) {
						constexpr float nameTextSize = 1.4f;
						constexpr float versionTextSize = 0.7f;
						static const float textHeight = (nameTextSize + versionTextSize * 0.7f /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::DEFAULT)->getLineHeight();
						constexpr float borderPadding = 1;
						constexpr float margin = 5;

						static std::string name = "Lunes Client";
#ifdef _DEBUG
						...
						//DrawUtils::fillRectangle(rect, MC_Color(255, 255, 255), .4);
						if (hudModule->textShadow)
							DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y + 1.065), &name, MC_Color(0, 0, 0), nameTextSize);
						DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(0, 0, 0), versionTextSize);
						if (hudModule->rainbow) {
					...
					if (moduleMgr->isInitialized() && shouldRenderArrayList) {
						...
						if (arrayMod->bottom)
							yOffset = windowSize.y - textHeight;  // needed for array list
						...
						struct IModuleContainer {
							...
							IModuleContainer(std::shared_ptr<IModule> mod) {
								...
								if (keybind == 0x0)
									...
									char text[50];
									sprintf_s(text, 50, "%s%s", moduleNameChr, arrayMod->keybinds ? std::string(" [" + std::string(Utils::getKeybindName(keybind)) + "]").c_str() : "");
									moduleName = text;
								}

								if (!this->enabled && *this->pos == vec2_t(0.f, 0.f))
									this->shouldRender = false;
								this->textWidth = DrawUtils::getTextWidth(&moduleName, arrayMod->scale);
							}
						...
						bool rainbowArray = true;
						static auto ThemeMod = moduleMgr->getModule<Theme>();
						static auto arrayMod = moduleMgr->getModule<ArrayList>();
						rainbowArray = arrayMod->rainbowArrayList && arrayMod->isEnabled();
						{
							auto lock = moduleMgr->lockModuleList();
							std::vector<std::shared_ptr<IModule>>* moduleList = moduleMgr->getModuleList();
							for (auto it : *moduleList) {
								//shit that doesnt display on the array list
								if (it.get() != hudModule && it.get() != arrayMod)
						...
						for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
							...
							DrawUtils::fillRectangle(rectPos, MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), ArrayList::opacity);
							if (!rainbowArray) {
								if (arrayMod->underline) {
									DrawUtils::fillRectangle(underline, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f);
								}
								if (arrayMod->outline) {
									DrawUtils::fillRectangle(leftRect, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f);
								}
							} else {
								if (arrayMod->outline) {
							...
							if (arrayMod->bottom) {
								yOffset -= textHeight + (textPadding * 2);  //change me to a - for going up and + for down
							} else {
		...
		// Draw Info / Alert Boxes
		{
			...
			if (box) {
				...
				DrawUtils::fillRectangle(rectPos, MC_Color(80, 80, 80), box->fadeVal);
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("Lunes Server");
			t->serverName2.setText("Lunes Server");  // This is the one actually displayed
			t->domain.setText(".Lunesbeta.club");
			t->serverAddress.setText("mc.Lunesbeta.club");
