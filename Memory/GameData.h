...
private:
	...
	std::queue<LunesDataPacket> LunesToInjectorQueue;
	std::map<int, std::function<void(std::shared_ptr<LunesDataPacket>)>> injectorToLunesResponseCallbacks;
	int lastRequestId = 0;
...
public:
	...
	void sendPacketToInjector(LunesDataPacket LunesDataPack);
	inline int addInjectorResponseCallback(std::function<void(std::shared_ptr<LunesDataPacket>)> callback) {
		lastRequestId++;
		injectorToLunesResponseCallbacks[lastRequestId] = callback;
		return lastRequestId;
	}
	void callInjectorResponseCallback(int id, std::shared_ptr<LunesDataPacket> packet);
	...
	inline void setInjectorConnectionActive(bool isActive) {
		if (injectorConnectionActive && !isActive) {
			std::queue<LunesDataPacket> empty;
			LunesToInjectorQueue.swap(empty);
		}
		injectorConnectionActive = isActive;
	};
	inline bool isPacketToInjectorQueueEmpty() { return LunesToInjectorQueue.empty(); };
	inline LunesDataPacket getPacketToInjector() {
		if (isPacketToInjectorQueueEmpty())
			throw std::exception("Packet send queue is empty");
		LunesDataPacket pk = LunesToInjectorQueue.front();
		LunesToInjectorQueue.pop();
		return pk;
	};
