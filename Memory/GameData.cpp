...
void GameData::sendPacketToInjector(LunesDataPacket LunesDataPack) {
	if (!isInjectorConnectionActive())
		throw std::exception("Lunes injector connection not active");
	if (LunesDataPack.dataArraySize >= 3000) {
		logF("Tried to send data packet with array size: %i %llX", LunesDataPack.dataArraySize, LunesDataPack.data.get());
		throw std::exception("Data packet data too big");
	}

	LunesToInjectorQueue.push(LunesDataPack);
}
void GameData::callInjectorResponseCallback(int id, std::shared_ptr<LunesDataPacket> packet) {
	if (injectorToLunesResponseCallbacks.find(id) == injectorToLunesResponseCallbacks.end()) {
		logF("No response callback for request with id=%i!", id);
		return;
	}
	injectorToLunesResponseCallbacks[id](packet);
	injectorToLunesResponseCallbacks.erase(id);
}
